...
def run_pipeline(name,alg,mrc,zdim,num_epochs,mask,poses=None,ctf=None,output_dir=None,
                 run_analysis=False,gt_dir=None,gt_latent=None,gt_labels=None,num_vols=None,
    ...
    if(alg == 'recovar'):
        command = f'recovar pipeline {starfile} --poses {poses} --ctf {ctf} --zdim {zdim} -o {output_path} --mask {mask} --correct-contrast --low-memory-option'
