...
from cov3d.fsc_utils import rpsd,upsample_and_expand_fourier_shell,average_fourier_shell
from cov3d.utils import get_torch_device
from tqdm import tqdm

def reconstruct_mean(dataset : CovarDataset,init_vol = None,mask=None,upsampling_factor=2,batch_size=1024):
    ...
    for i in tqdm(range(0,len(dataset),batch_size),desc='Reconstructing mean volume'):
        images,pts_rot,filter_indices,_ = dataset[i:min(i + batch_size,len(dataset))]
...
if __name__ == "__main__":
    ...
    rec_vol = reconstruct_mean(dataset,vol,batch_size=2048,mask=mask)
    #rec_vol = reconstruct_mean(dataset,batch_size=2048)
